...
const LostRequests = () => {
            ...
            if (confirmOrdersData) {
                ...
                Object.entries(confirmOrdersData).forEach(([userId, userConfirmOrders]) => {
                    Object.entries(userConfirmOrders).forEach(([bookId, confirmOrder]) => {
                        if (confirmOrder.lost) {
                            const lostRequest = {
                                userId,
                                bookId,
                                lostTime: confirmOrder.lostTime,
                            ...
                            lostRequestsList.push(lostRequest);
                        }
                    });
                });

                setLostRequests(lostRequestsList);
            }
        });
    ...
    const handleApprove = (request) => {
        ...
        // Remove the 'lost' field from the ConfirmOrders node
        ...
        update(confirmOrderRef, updates)
            .then(() => {
                console.log('Lost field removed from ConfirmOrders.');
            })
            .catch((error) => {
                console.error('Error removing lost field from ConfirmOrders:', error);
            });
        ...
        // Update the UploadBooks node with the lost field
        ...
        update(uploadBookRef, uploadBookUpdates)
            .then(() => {
                console.log('Lost field added to UploadBooks.');
            })
            .catch((error) => {
                console.error('Error adding lost field to UploadBooks:', error);
            });
    ...
    return (
        <div className='my-4 border-t border-gray-300 py-4'>
            <h1 className='text-2xl font-bold'>{lostRequests.length > 0 ? <>Lost Requests : {lostRequests.length}</> : 'No Lost Requests'}</h1>
            {lostRequests.length > 0 ? (
                lostRequests.map((request, index) => (
                    <div key={index} className='border px-4 py-3 rounded-xl my-4'>
                        ...
                        <p>Lost Date: {formatTimestamp(request.lostTime)}</p>
